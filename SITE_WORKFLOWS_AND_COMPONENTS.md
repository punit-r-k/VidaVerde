...
- Behavior:
  ...
  - A valid shipping address triggers concurrent Shipping and Expedited live-rate previews whenever the cart or address changes; both final charges and expected arrival dates appear together, method changes use the already-loaded previews, and stale browser responses are ignored.
  - Expedited is offered only when it is both faster and more expensive than Shipping. A cheaper or equal faster label is assigned to Shipping and Expedited is hidden. Every parcel in a multi-parcel quote must use the same carrier.
  - Before a valid address finishes rating, the checkout displays only Shipping. Expedited is added only after the calculated rates satisfy the premium-option policy.
  - `POST /api/order` creates a Stripe PaymentIntent after calculating the authoritative live shipping charge.
  - Stripe’s Payment Element collects payment details; the app never handles raw card data.
  - Customers choose Shipping (3–5 business days) or Expedited Shipping (1–3 business days). EasyPost selects the fastest rate inside the chosen window, the website shows the expected arrival date, and the same saved quote is reused for automatic prepaid-label purchase after payment.
...
- Behavior:
  - Stripe webhook verifies signatures and processes `payment_intent.succeeded` for this integration.
  - Checkout Session events are no longer accepted.
  ...
  - Shipping preorders do not create or purchase labels until every preorder unit is ready.
  - Confirmation emails show readable tracking numbers when a label is available.
...
- Behavior:
  ...
  - Restocks create durable release events; a permanent five-minute worker claims and sends consolidated preorder-ready emails.
  ...
  - Concurrent workers cannot claim the same release events, and stale claims recover after their lease expires.
...
- Behavior:
  ...
  - Shipment refresh is an explicit authenticated `POST`; the following `GET` only reads reporting data.
...
| `lib/preorderReadyEmail.js` | Preorder release email composition, grouping, and delivery for pickup and shipping orders. |
| `app/api/order/route.js` | Checkout validation, authoritative pricing, and Stripe PaymentIntent creation. |
