...
### 5. Market Pickup Scheduling And Policy
...
- Behavior:
  - Pulls paid orders for the current market collection window.
  - Separates ready-to-produce jars from preorder quantities.
...
| `app/api/admin/restock/route.js` | Admin restock delta endpoint. |
| `app/api/admin/prep/route.js` | Weekly production and pickup-prep dataset builder. |
...
| `supabase/schema.sql` | Core schema, RPCs, and data model for products, inventory, orders, and analytics. |
