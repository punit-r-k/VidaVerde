...
function StripePaymentForm({
  ...
  const payNowLabel =
    Number.isFinite(Number(grossTotalCents)) && Number(grossTotalCents) > 0
  ...
  const handleStripeSubmit = async () => {
    ...
    if (paymentMethodError || !paymentMethod?.id) {
      const message =
        paymentMethodError?.message || "Enter valid card details to continue.";
      setPaymentError(message);
      onPaymentState({ status: "error", message });
    ...
    if (error) {
      const message =
        error.message || "Payment didn't go through. Please check your card and try again.";
      setPaymentError(message);
      onPaymentState({ status: "error", message });
      setIsSubmitting(false);
      return;
