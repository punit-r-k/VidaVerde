...
export const DEFAULT_SHIPPING_OPTION_ID = "regular";
export const OWNER_DELIVERY_SHIPPING_OPTION_ID = "owner_delivery";
export const SHIPPING_OPTIONS = [
  {
    id: DEFAULT_SHIPPING_OPTION_ID,
    label: "Regular shipping",
    amountCents: 999,
    transitLabel: "3-5 days",
  ...
  },
  {
    ...
    amountCents: 1499,
    transitLabel: "1-3 days",
    note: "Faster shipping window."
  },
  {
    id: OWNER_DELIVERY_SHIPPING_OPTION_ID,
    label: "Personal delivery by the owner",
    amountCents: 100000,
    transitLabel: "2-7 days",
    note: "Greater Houston-area TX ZIP codes only."
  }
];
...
export const normalizeShippingOptionId = (value) => {
  ...
  if (normalized === "expidited") return "expedited";
...
export const isOwnerDeliveryAddressEligible = ({ state, postalCode } = {}) => {
  const normalizedState = formatState(state);
  const postalPrefix = formatPostalCode(postalCode).slice(0, 3);
  return normalizedState === "TX" && GREATER_HOUSTON_ZIP_PREFIXES.has(postalPrefix);
};
