...
export const SHIPPING_OPTIONS = [
  {
    ...
    amountCents: 999,
    transitLabel: "Ships on the next packing day, then typically arrives in 2–5 business days.",
    carrierTransitLabel: "2–5 business days",
    note: "Final rate is based on the cart."
  },
  {
    id: EXPEDITED_SHIPPING_OPTION_ID,
    label: "Expedited Shipping",
    amountCents: 1499,
    transitLabel: "Ships on the next packing day, then typically arrives in 1–3 business days.",
...
const SHIPPING_OPTION_IDS = SHIPPING_OPTIONS.map((option) => option.id);
const SHIPPING_OPTION_MAP = new Map(
  SHIPPING_OPTIONS.map((option) => [option.id, option])
);

export const normalizeShippingOptionId = (value) => {
  const normalized = normalizeSharedShippingOptionId(value);
  return SHIPPING_OPTION_MAP.has(normalized) ? normalized : DEFAULT_SHIPPING_OPTION_ID;
};

export const getShippingOptionById = (value) =>
  SHIPPING_OPTION_MAP.get(normalizeShippingOptionId(value)) ||
  SHIPPING_OPTION_MAP.get(DEFAULT_SHIPPING_OPTION_ID);
...
export const isOwnerDeliveryAddressEligible = (address = {}) =>
  isGreaterHoustonShippingAddressEligible({
    ...address,
...
export const checkoutPayloadSchema = z
  ...
  .superRefine((payload, context) => {
    ...
    if (payload.customer.country !== SHIPPING_COUNTRY_CODE) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (!payload.customer.state) {
      ...
      const suggestion = getStateSpellingSuggestion(payload.customer.state);
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["customer", "state"],
        message: suggestion
    ...
    if (
      US_STATE_CODES.has(payload.customer.state) &&
      POSTAL_REGEX.test(payload.customer.postalCode) &&
      !isGreaterHoustonShippingAddressEligible(payload.customer)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["customer", "postalCode"],
        message: LOCAL_SHIPPING_AREA_ERROR_MESSAGE
      });
    }

    if (
      payload.shippingOption === OWNER_DELIVERY_SHIPPING_OPTION_ID &&
      !isOwnerDeliveryAddressEligible(payload.customer)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["shippingOption"],
        message:
          "Personal delivery by the owner is only available in Greater Houston-area TX ZIP codes."
      });
    }
  });
