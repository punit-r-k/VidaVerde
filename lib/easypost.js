...
export const isEasyPostTestMode = () =>
  /^EZTK/i.test(String(process.env.EASYPOST_API_KEY || "").trim());
...
export const createAndVerifyEasyPostAddress = async (address, { timeoutMs } = {}) => {
  const response = await easyPostRequest("/addresses", {
    method: "POST",
    timeoutMs,
    body: { address, verify_strict: true }
  });
  const verified = response?.address || response;
  if (!verified?.id || verified?.verifications?.delivery?.success !== true) {
