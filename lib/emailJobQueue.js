...
import { sendShipmentTrackingEmail } from "@/lib/shipmentTrackingEmail";
import { supabaseAdmin } from "@/lib/supabaseAdmin";

const ORDER_CONFIRMATION_JOB_TYPE = "order_confirmation";
const SHIPMENT_TRACKING_JOB_TYPE = "shipment_tracking";
// Customer-facing shipment tracking emails are intentionally disabled. Carrier
// tracking numbers remain stored internally with purchased labels.
const SUPPORTED_JOB_TYPES = [ORDER_CONFIRMATION_JOB_TYPE];
...
export const buildShipmentTrackingMessageId = (shipmentId) => {
  const normalizedShipmentId = toText(shipmentId, 64)
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
  if (!normalizedShipmentId) return "";
  return `<shipment-tracking.${normalizedShipmentId}@${getMessageIdDomain()}>`;
};
...
export async function enqueueShipmentTrackingEmail({ shipmentId } = {}) {
  void shipmentId;
  return { ok: false, disabled: true, error: "Shipment tracking emails are disabled." };
}
...
const selectPendingEmailJobs = async (limit) => {
  ...
  const [pendingResult, staleResult] = await Promise.all([
    supabaseAdmin
      .from("email_jobs")
      .select("id, type, available_at, created_at")
...
const claimEmailJob = async (candidate, staleBeforeIso) => {
  const claimToken = randomUUID();
  const jobId = toText(candidate?.id, 36);
  const type = toText(candidate?.type, 40);
  const rpcName = type === SHIPMENT_TRACKING_JOB_TYPE
    ? "claim_shipment_tracking_email_job"
    : "claim_email_job";
  ...
  return {
    ...
    job: {
      ...
      message_id:
        ...
        (job.type === SHIPMENT_TRACKING_JOB_TYPE
          ? buildShipmentTrackingMessageId(job.shipment_id || job?.payload?.shipmentId)
...
const completeEmailJob = async (job) => {
  const rpcName = job?.type === SHIPMENT_TRACKING_JOB_TYPE
    ? "complete_shipment_tracking_email_job"
    : "complete_email_job";
  const { data, error } = await supabaseAdmin.rpc(rpcName, {
    p_job_id: toText(job?.id, 36),
    p_claim_token: toText(job?.claim_token, 36),
...
const processEmailJob = async (candidate, staleBeforeIso) => {
  const claim = await claimEmailJob(candidate, staleBeforeIso);
  if (!claim.ok) {
    return {
      ok: false,
      skipped: false,
  ...
  const job = claim.job;
  if (!SUPPORTED_JOB_TYPES.includes(job.type) || !job.payload || typeof job.payload !== "object") {
  ...
  let sendResult;
  try {
    sendResult = job.type === SHIPMENT_TRACKING_JOB_TYPE
      ? await sendShipmentTrackingEmail(job.payload, { messageId: job.message_id })
      : await sendOrderConfirmationEmail(job.payload, { messageId: job.message_id });
  } catch (error) {
    sendResult = {
      ok: false,
...
export async function processOrderConfirmationEmailJob(jobId) {
  ...
  return processEmailJob(
    { id: normalizedJobId, type: ORDER_CONFIRMATION_JOB_TYPE },
    toStaleBeforeIso()
  );
}

export async function processShipmentTrackingEmailJob(jobId) {
  void jobId;
  return { ok: false, disabled: true, outcome: "disabled" };
}
...
export async function retryFailedEmailJobs({ limit = 10 } = {}) {
  ...
  for (const row of data || []) {
    const retryRpc = row?.type === SHIPMENT_TRACKING_JOB_TYPE
      ? "retry_failed_shipment_tracking_email_job"
...
export async function getEmailJobStatusCounts({ staleBefore = "" } = {}) {
  ...
  for (const status of ["pending", "failed"]) {
    const { count, error } = await supabaseAdmin
      .from("email_jobs")
      .select("id", { count: "exact", head: true })
      .eq("type", SHIPMENT_TRACKING_JOB_TYPE)
      .eq("status", status);
    if (error) {
      errors.push(error);
      counts[`tracking${status === "pending" ? "Pending" : "Failed"}`] = null;
    } else {
      counts[`tracking${status === "pending" ? "Pending" : "Failed"}`] = count || 0;
    }
  }
