...
export const resolveOrderConfirmationTrackingState = ({
  ...
  if (isTestOrder) return ORDER_CONFIRMATION_TRACKING_STATES.TEST_NOT_APPLICABLE;
...
export async function sendOrderConfirmationEmail(order, { messageId = "" } = {}) {
  ...
  const trackingResult =
    authoritativeOrder.fulfillment === "ship" && !orderContext.isTestOrder
  ...
  if (trackingResult.error) {
    return {
      ok: false,
      code: "dependency_unavailable",
      error: trackingResult.error.message || "Shipment tracking lookup failed."
    };
  }
  ...
  if (
    ...
    return {
      ...
      error: "Shipment tracking is not ready yet."
    };
  }
