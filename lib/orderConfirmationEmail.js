...
const SHIPPING_PREORDER_ONLY_TIMING_NOTICE =
  "Pre-order timing is not guaranteed because fermentation and restock timing can vary. Once the order is ready, it will be included in the next Wednesday carrier handoff. Carrier transit estimates begin after the carrier receives the package.";
...
const buildShippingPreorderFollowUpMessage = () =>
  "We will send another email once your pre-ordered items are restocked, then prepare your shipment and send tracking or delivery updates when available.";
...
const buildShippingLines = (
  customer,
  addressLines,
  { estimatedShipDateText = "", hasPreorderItems = false } = {}
) => {
  const method = toText(customer?.shipping_option_label, 120);
  const transit = toText(customer?.shipping_estimate, 180);
  ...
  const lines = [
    ...
    transit ? `Shipping timing: ${transit}` : ""
  ].filter(Boolean);
...
const buildEmailContent = ({
  ...
  const shippingMethodLabel = toText(customer?.shipping_option_label, 120);
  const shippingTransitLabel = toText(customer?.shipping_estimate, 180);
              ...
              <tbody>
                ...
                ${
                  shippingTransitLabel
                    ? `
                      <tr>
                        <td style="padding: 4px 0; color: #6b7280; width: 160px;">Shipping timing</td>
...
export async function sendOrderConfirmationEmail(order) {
  ...
  const pickupCalendarInvite =
    order?.fulfillment === "market" && pickupContext.pickupScenario !== "preorder_only"
