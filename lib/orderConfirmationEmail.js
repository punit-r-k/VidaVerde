...
const getOrderPreorderStatus = async (orderId) => {
  ...
  const status = (data || []).reduce(
    (acc, item) => {
      ...
      const preorderQty = Math.max(Number.parseInt(item?.preorder_qty, 10) || 0, 0);
  ...
  const fulfillmentCopy =
    fulfillment === "market"
      ? pickupScenario === "mixed"
        ? "Your reservation items are confirmed for pickup on the date below. Your pre-ordered items will likely be ready on a different market day, and we will email you again when they come back in stock."
