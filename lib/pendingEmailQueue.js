...
const getFailedConfirmationAction = (code) => {
  switch (toText(code, 80).toLowerCase()) {
    ...
    default:
      return "Use Retry Failed Emails.";
  }
};
...
const buildConfirmationQueueRow = (job, nowMs) => {
  ...
  const isShipmentTracking = toText(job?.type, 40) === "shipment_tracking";
  ...
  return {
    id: `${isShipmentTracking ? "tracking" : "confirmation"}:${toText(job?.id, 36) || orderId}`,
    queued_at: toIso(job?.created_at),
    recipient,
    email_for: `${isShipmentTracking ? "Shipment tracking" : "Order confirmation"}${isTestOrder ? " (test order)" : ""}`,
