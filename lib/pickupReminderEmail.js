...
const isFridayOrderInMarketTimezone = (createdAt, timeZone = MARKET_TIMEZONE) => {
...
const buildReminderEmailContent = ({ order, pickupDetails }) => {
  ...
  const contactLine =
    `If you cannot make it to the market, call or text ${SUPPORT_PHONE_DISPLAY} as soon as possible.`;
  ...
  const html = `
    <div style="font-family: 'Trebuchet MS', Tahoma, sans-serif; color: #1f2937; line-height: 1.6;">
      ...
      <p style="margin-top: 24px; margin-bottom: 0;">
        If you cannot make it to the market, call or text
...
const buildReminderCandidates = async ({ now = new Date() } = {}) => {
  ...
  const [itemsResult, releasedResult] = await Promise.all([
    supabaseAdmin
      .from("order_items")
      .select(
        "order_id, sku, quantity, preorder_qty, products(name), orders!inner(id, fulfillment, created_at, status, customer_name, customer_email, customer_phone, pickup_reminder_email_sent_at)"
      )
  ...
  for (const row of itemsResult.data || []) {
    ...
    if (Number.isFinite(collectionStartMs) && createdAtMs < collectionStartMs) {
      continue;
    }
    if (Number.isFinite(collectionEndMs) && createdAtMs >= collectionEndMs) {
      continue;
    }
