...
const getShipmentTrackingNumbers = async (shipmentId) => {
  const normalizedShipmentId = toText(shipmentId, 36);
  if (!normalizedShipmentId) {
    return { numbers: [], error: new Error("Shipment ID is required for tracking lookup.") };
  }
  if (!supabaseAdmin) {
  ...
  try {
    const { data, error } = await supabaseAdmin
      .from("shipments")
      .select("tracking_number, shipment_parcels(parcel_index, tracking_number)")
    ...
    console.error("preorder ready tracking lookup error:", error);
    return {
      numbers: [],
...
export const buildReadyEmailContent = ({
  ...
  const trackingNotApplicable = isFinalShipping && isTestOrder;
  ...
  const partialOrderNotice = hasRemainingPreorders
    ...
    : isShipping
      ? trackingNotApplicable
        ...
        : "Everything that was pre-ordered for this order is now ready. Your shipment tracking is shown below."
    ...
    isShipping
      ? hasRemainingPreorders
        ...
        : trackingNotApplicable
          ...
          : `Your shipment is already being prepared for this address. If the address is wrong, text ${SUPPORT_PHONE_DISPLAY} immediately. Changes may no longer be possible.`
  ...
  const fulfillmentDetailsText = isShipping
    ? [
        ...
        `${hasRemainingPreorders ? "Estimated ship date after preorder window" : "Estimated ship date"}: ${shippingMethod.shipDateLabel}`,
        `Expected arrival: ${shippingMethod.expectedArrivalLabel}`,
        "When it arrives: Open and inspect the package, then refrigerate after opening.",
      ...
      ? hasRemainingPreorders
        ...
        : trackingNotApplicable
          ...
          : trackingNumbers.length > 0
            ...
            : "Shipment tracking is unavailable."
            ...
            <table role="presentation" style="width: 100%; border-collapse: collapse;">
              <tbody>
                ${
                  ...
                  <tr>
                    <td style="padding: 4px 0; color: #6b7280; width: 160px;">${hasRemainingPreorders ? "Estimated ship date after preorder window" : "Estimated ship date"}</td>
                  ...
                  <tr>
                    ...
                    <td style="padding: 4px 0;">Open and inspect the package, then refrigerate after opening.</td>
                  </tr>`
                }
                ${
                  addressLines.length > 0
                      ...
                      trackingNotApplicable
                        ...
                        : trackingNumbers.length > 0
                          ? trackingNumbers
                              ...
                              .join("")
                          : '<span style="display: block; color: #374151;">Tracking is unavailable.</span>'
                    }
                  </div>
                `
                : ""
...
const prepareShippingOrder = async (order, { hasRemainingPreorders }) => {
  ...
  const { data: shipmentId, error } = await supabaseAdmin.rpc("sync_shipment_for_order", {
    p_order_id: order.orderId
  });
  ...
  try {
    ...
    const trackingResult = await getShipmentTrackingNumbers(shipmentId);
    if (trackingResult.error) throw trackingResult.error;
    if (trackingResult.numbers.length === 0) {
      throw new Error("Shipment tracking is not ready yet.");
    }

    order.trackingNumbers = trackingResult.numbers;
    return { ok: true };
  } catch (error) {
