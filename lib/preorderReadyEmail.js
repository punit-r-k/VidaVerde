...
export async function sendPreorderReadyEmails({ sku, now = new Date() } = {}) {
  ...
  const cutoff = new Date(now).getTime() - PREORDER_READY_EMAIL_DELAY_MS;
  if (!Number.isFinite(cutoff)) {
    return { ok: false, error: "The preorder-ready email time is invalid." };
  }

  let readyEventsQuery = supabaseAdmin
    .from("preorder_release_events")
    .select(
    ...
    .lte("created_at", new Date(cutoff).toISOString());

  if (cleanSku) {
    readyEventsQuery = readyEventsQuery.eq("sku", cleanSku);
  }
