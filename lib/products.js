...
import {
  PRODUCT_TYPE_HOT_SAUCE,
  PRODUCT_TYPE_SAUERKRAUT,
  normalizeProductType
} from "./shippingPricing";

const PRODUCT_IMAGE_BY_SKU = {
  VV1: "/product-photos/Red-Coral.webp",
  VV2: "/product-photos/Sunset.webp",
  VV3: "/product-photos/Caribbean-Heat.webp",
  VV4: "/product-photos/Endless-Summer.webp",
  VV5: "/product-photos/Green-Kick.webp",
  VV6: "/product-photos/Hell-Yeah.webp"
};

const FALLBACK_PRODUCT_IMAGE = "/email/order-confirmation-banner.png";

const getFallbackProductImage = (sku) =>
  PRODUCT_IMAGE_BY_SKU[sku] || FALLBACK_PRODUCT_IMAGE;
...
const FALLBACK_PRODUCTS = [
  {
    ...
    image: getFallbackProductImage("VV1"),
    priceCents: 1199,
    productType: PRODUCT_TYPE_SAUERKRAUT,
    sizeOz: 12
  },
  {
    id: "citrus-fennel",
    ...
    image: getFallbackProductImage("VV2"),
    priceCents: 1199,
    productType: PRODUCT_TYPE_SAUERKRAUT,
    sizeOz: 12
  },
  {
    ...
    image: getFallbackProductImage("VV3"),
    priceCents: 1199,
    productType: PRODUCT_TYPE_SAUERKRAUT,
    sizeOz: 12
  },
  {
    ...
    image: getFallbackProductImage("VV4"),
    priceCents: 1199,
    productType: PRODUCT_TYPE_SAUERKRAUT,
    sizeOz: 12
  },
  {
    ...
    image: getFallbackProductImage("VV5"),
    priceCents: 999,
    productType: PRODUCT_TYPE_HOT_SAUCE,
    sizeOz: 5
  },
  {
    id: "hell-yeah",
    ...
    image: getFallbackProductImage("VV6"),
    priceCents: 999,
    productType: PRODUCT_TYPE_HOT_SAUCE,
    sizeOz: 5
  }
...
const mapProductRow = (row) => {
  ...
  const imageUrl =
    typeof row.image_url === "string" && row.image_url.trim()
      ? row.image_url
      : getFallbackProductImage(row.sku);
  ...
  return {
    ...
    productType: normalizeProductType(row.product_type, row.sku),
...
export async function getProducts() {
  ...
  const { data, error } = await supabaseAdmin
    .from("products")
    .select(
      "id, sku, name, profile, description, specs, image_url, price_cents, product_type, size_oz, sort_order"
    )
    .eq("active", true)
...
const loadActiveProductMap = async (skus) => {
  if (!supabaseAdmin) {
  ...
  try {
    const { data, error } = await supabaseAdmin
      .from("products")
      .select("sku, name, price_cents, image_url, product_type")
      .in("sku", skus)
      .eq("active", true);
...
export async function getAuthoritativeProductMap(skus) {
  if (!Array.isArray(skus) || skus.length === 0) {
    return new Map();
  }

  const result = await loadActiveProductMap(skus);
...
export async function getProductMap(skus) {
  ...
  const result = await loadActiveProductMap(skus);
  return result.ok && result.map.size > 0
    ? result.map
    : fallbackProductMap(skus);
}
