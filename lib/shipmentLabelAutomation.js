...
const getSavedAutomaticQuote = async (shipmentId) => {
  ...
  if (quoteError) throw quoteError;
  return quoteUsesOneCarrier(quote) ? quote : null;
};
...
const getCheckoutQuote = async (shipment) => {
  ...
  if (savedQuote) return quoteUsesOneCarrier(savedQuote) ? savedQuote : null;
  ...
  if (!checkoutQuote?.quote_json || !quoteUsesOneCarrier({
