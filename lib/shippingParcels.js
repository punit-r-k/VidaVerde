...
const MAX_SHIPPING_PLANS = 6;
const MAX_RATED_PARCELS_PER_QUOTE = 12;
...
export function buildShippingPlans(items) {
  ...
  const selectedPlans = [];
  let ratedParcelCount = 0;
  for (const plan of combinedPlans) {
    if (selectedPlans.length >= MAX_SHIPPING_PLANS) break;
    if (ratedParcelCount + plan.parcels.length > MAX_RATED_PARCELS_PER_QUOTE) continue;
    selectedPlans.push(plan);
    ratedParcelCount += plan.parcels.length;
  }
