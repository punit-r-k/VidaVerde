...
export const DEFAULT_SHIPPING_OPTION_ID = FASTEST_SHIPPING_OPTION_ID;
...
export const SHIPPING_SCHEDULE_STANDARD_LINE =
  "We automatically choose the fastest available EasyPost carrier service for your address.";
export const SHIPPING_SCHEDULE_EXPEDITED_LINE =
  "You pay the live carrier postage plus the exact packaging cost, rounded up to the nearest dollar.";
export const SHIPPING_SCHEDULE_EXPEDITED_NOTE =
  "Transit time begins only after the carrier receives the package on our Wednesday handoff day.";
...
const FASTEST_ESTIMATE = {
  label: "Handed to the carrier on Wednesday using the fastest available service for your address.",
...
export const getShippingOptionsForCart = ({ items = [], subtotalCents = 0 } = {}) => {
  ...
  const fastestOption = {
    ...
    amountCents: 0,
    transitLabel: FASTEST_ESTIMATE.label,
    carrierTransitLabel: FASTEST_ESTIMATE.carrierTransitLabel,
    note: "Live postage plus packaging is calculated for the entered address.",
    deliveryEstimate: FASTEST_ESTIMATE.deliveryEstimate
  };
...
export const normalizeShippingOptionId = (value) => {
  ...
  if ([
    ...
    "personal_delivery",
    "owner"
  ].includes(normalized)) return FASTEST_SHIPPING_OPTION_ID;
