...
export const getEasyPostBudgetConfig = () => ({
  dailyRatingLimit: envInt("EASYPOST_DAILY_RATING_LIMIT", 100),
  dailyAddressVerificationLimit: envInt("EASYPOST_DAILY_ADDRESS_VERIFICATION_LIMIT", 100),
  monthlyOverageLimitCents: envInt("EASYPOST_MONTHLY_OVERAGE_LIMIT_CENTS", 500),
  ratingOverageUnitCents: envInt("EASYPOST_RATING_OVERAGE_UNIT_CENTS", 1),
...
export const finalizeReservedShippingQuote = async ({ quoteId, selection, fingerprint }) => {
  if (!supabaseAdmin) throw new Error("Shipping quote storage is unavailable.");
  ...
  const firstRate = quote?.parcels?.[0]?.selectedRate || {};
  const { data, error } = await supabaseAdmin
    ...
    .update({
      ...
      verified_address_id: String(selection?.verifiedAddressId || "") || null,
