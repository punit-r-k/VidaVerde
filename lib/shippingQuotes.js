...
export const createEasyPostRatingContext = ({ deadlineAt } = {}) => {
  const startedAt = new Date();
  const estimatedShipDate = getEstimatedShipDate({ orderDate: startedAt });
  const defaultDeadlineAt = startedAt.getTime() + EASYPOST_RATING_BUDGET_MS;
  ...
  for (const plan of plans) {
    ...
    for (const [parcelIndex, parcel] of plan.parcels.entries()) {
      const easyPostShipment = await getRatedEasyPostShipment(ratingContext, {
        toAddress: verifiedToAddress.id,
        planKey: plan.key,
        parcelIndex,
        parcel,
        options
