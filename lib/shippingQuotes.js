...
export async function getEasyPostQuotes(shipment) {
  ...
  for (const plan of plans) {
    ...
    for (const parcel of plan.parcels) {
      ...
      const rates = (easyPostShipment.rates || []).map(normalizeRate)
        ...
        .map((rate) => ({
          ...rate,
          meetsDeliveryTarget: rate.deliveryDays === null || rate.deliveryDays <= deadline
        }))
        .sort((a, b) => Number(b.meetsDeliveryTarget) - Number(a.meetsDeliveryTarget) || a.amountCents - b.amountCents);
      if (!rates.length) throw new Error(`No EasyPost rate was returned for ${parcel.packageCode}.`);
  ...
  quotes.sort((a, b) => {
    const aMeetsTarget = a.parcels.every((parcel) => parcel.selectedRate.meetsDeliveryTarget);
    const bMeetsTarget = b.parcels.every((parcel) => parcel.selectedRate.meetsDeliveryTarget);
    return Number(bMeetsTarget) - Number(aMeetsTarget) || a.totalCostCents - b.totalCostCents;
  }).forEach((quote) => {
  ...
  const rankedQuotes = [...uniqueQuotes.values()];
  if (shipment.shipping_option === "expedited") {
    const expeditedQuotes = rankedQuotes.filter((quote) =>
      quote.parcels.every((parcel) => parcel.selectedRate.meetsDeliveryTarget)
    );
    return (expeditedQuotes.length ? expeditedQuotes : rankedQuotes).slice(0, 5);
  }
  return rankedQuotes.slice(0, 5);
}
