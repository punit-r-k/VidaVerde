...
export const filterRatesForTransitWindow = (
  rates,
  transitWindow
) => {
  const candidates = Array.isArray(rates) ? rates : [];
  if (!transitWindow) return candidates;

  const preferredRates = candidates.filter((rate) =>
    isRateWithinTransitWindow(rate, transitWindow)
  );
  if (preferredRates.length > 0) return preferredRates;

  const minimum = Number(transitWindow.minimumDays);
  const maximum = Number(transitWindow.maximumDays);
  if (!Number.isFinite(minimum) || !Number.isFinite(maximum)) return [];

  // This fallback only filters rates already returned by the existing rating
  // response. It must remain local so it cannot consume another provider call.
  // Use a faster estimate only when no rate is inside the preferred window so
  // standard shipping does not unnecessarily select premium service.
  return candidates.filter((rate) => {
    const transitDays = getRateTransitDays(rate);
    return transitDays !== null && transitDays < minimum && transitDays <= maximum;
  });
};
...
export const getSameCarrierRateSets = (ratesByParcel = []) => {
  const parcelRates = Array.isArray(ratesByParcel)
    ? ratesByParcel.map((rates) => Array.isArray(rates) ? rates : [])
    : [];
  if (parcelRates.length === 0 || parcelRates.some((rates) => rates.length === 0)) {
    return [];
  }

  const carrierKeys = [...new Set(
    parcelRates[0]
      .map((rate) => normalizeCarrierKey(rate?.carrier))
      .filter(Boolean)
  )].filter((carrierKey) =>
    parcelRates.every((rates) =>
      rates.some((rate) => normalizeCarrierKey(rate?.carrier) === carrierKey)
    )
  );

  return carrierKeys.map((carrierKey) =>
    parcelRates.map((rates) =>
      [...rates]
        .filter((rate) => normalizeCarrierKey(rate?.carrier) === carrierKey)
        .sort(compareRatesBySpeed)[0]
    )
  );
};
