...
create table if not exists checkout_shipping_quotes (
  ...
  check (charged_shipping_cents = unrounded_cents + rounding_cents - discount_cents),
  check (discount_cents <= unrounded_cents + rounding_cents),
...
begin
  ...
  if not found then raise exception 'Unknown order id: %', p_order_id; end if;
  if not coalesce(v_order.is_test_order, false) then
    return public.sync_shipment_for_order(p_order_id);
  end if;
  if v_order.fulfillment <> 'ship' or v_order.status <> 'paid' then return null; end if;
  if exists (
    select 1 from preorder_queue pq
    where pq.order_id = v_order.id and pq.remaining > 0
  ...
  where s.order_id = v_order.id
    and (
      ...
      or nullif(trim(coalesce(s.tracking_number, '')), '') is not null
      or exists (
        select 1 from shipment_parcels sp
        where sp.shipment_id = s.id
          and sp.status in ('label_purchased', 'shipped', 'delivered')
      )
    );
  ...
  update shipments s
  set status = 'pending_label', label_purchase_started_at = null,
      label_purchase_error = null, updated_at = clock_timestamp()
  where s.order_id = v_order.id and s.status = 'cancelled'
    and s.label_purchased_at is null
    and nullif(trim(coalesce(s.label_url, '')), '') is null
    and nullif(trim(coalesce(s.tracking_number, '')), '') is null
    and not exists (
  ...
  where oi.order_id = v_order.id;
  if v_item_count <= 0 then raise exception 'Shipping order % has no items', p_order_id; end if;
  ...
  insert into shipments (
    ...
    v_item_count, coalesce(v_order.amount_total, 0), upper(coalesce(v_order.currency, 'USD')),
    nullif(v_order.shipping_tier, ''), nullif(v_order.shipping_option, ''),
    nullif(v_order.shipping_option_label, ''), nullif(v_order.shipping_estimate, ''),
    coalesce(v_order.sauerkraut_count, 0), coalesce(v_order.hot_sauce_count, 0), null
  )
  on conflict (order_id) do update set
    status = case
      ...
      then 'pending_label' else shipments.status end,
    label_purchase_started_at = null, label_purchase_error = null,
    items_summary = excluded.items_summary, items_json = excluded.items_json,
    item_count = excluded.item_count, updated_at = clock_timestamp()
  returning id into v_shipment_id;
  return v_shipment_id;
end;
$$;
...
begin
  for v_order_id in
    ...
    where o.fulfillment = 'ship' and o.status = 'paid'
      and coalesce(o.is_test_order, false)
      and not exists (
        select 1 from preorder_queue pq
      ...
      and (
        ...
        or exists (
          select 1 from shipments s where s.order_id = o.id
            and s.status = 'cancelled' and s.label_purchased_at is null
            and nullif(trim(coalesce(s.label_url, '')), '') is null
            and nullif(trim(coalesce(s.tracking_number, '')), '') is null
        )
      )
  ...
  loop
    ...
    if v_shipment_id is not null then v_synced := v_synced + 1; end if;
  end loop;
  return v_synced;
end;
$$;
...
begin
  foreach v_signature in array array[
    ...
    'public.sync_shipment_for_order_with_test_labels(uuid)',
    'public.sync_test_shipments_for_labels()',
